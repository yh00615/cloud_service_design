...
> [!NOTE]
> EventBridge Event Bus는 CloudFormation에 포함되지 않습니다. 태스크 1에서 학생이 직접 생성합니다.
...
> [!NOTE]
> Event Bus는 CloudFormation 출력값에 포함되지 않습니다. 태스크 1에서 학생이 직접 생성하며, 이름은 `QuickTableReservationEventBus`를 사용합니다.
...
2. Event bus: `QuickTableReservationEventBus` 선택
...
> [!NOTE]
> CloudFormation 스택을 삭제하면 DynamoDB 테이블, Lambda 함수, IAM 역할, SNS Topic 등 모든 리소스가 자동으로 삭제됩니다. Event Bus는 별도로 삭제해야 합니다.
