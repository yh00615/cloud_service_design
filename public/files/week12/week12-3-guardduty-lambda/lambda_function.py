...
def lambda_handler(event: dict, context: Any) -> dict:
    ...
    try:
        ...
        # 처리 시작 로그 출력
        logger.info(f"Processing Finding: {finding_type}")
...
def get_or_create_isolation_sg(vpc_id: str) -> str:
    """
    격리 보안 그룹을 조회하거나 생성
    ...
    try:
        ...
        # 오류 발생 시 로그 출력 및 예외 재발생
        logger.error(f"Error managing isolation security group: {str(e)}")
        raise


def send_notification(finding_type: str, severity: float, title: str, 
                     description: str, instance_id: Optional[str], 
