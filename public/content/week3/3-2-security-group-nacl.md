...
> [!NOTE]
> AWS CloudFormation 스택 목록 페이지로 자동 이동합니다. 스택 생성이 시작되면 상태가 "CREATE_IN_PROGRESS"로 표시됩니다. 스택 생성에 5-7분이 소요됩니다. **Events** 탭에서 생성 과정을 확인할 수 있습니다. 대기하는 동안 다음 태스크를 미리 읽어봅니다. 상태가 "**CREATE_COMPLETE**"로 변경될 때까지 기다립니다.

18. **Outputs** 탭을 선택합니다.
19. 출력값들을 확인하고 다음 5개 값을 메모장에 복사합니다:
    - `VpcId`: Amazon VPC ID
...
20. AWS Management Console에서 상단 검색창에 `VPC`을 입력하고 선택합니다.
21. 왼쪽 메뉴에서 **Security groups**를 선택합니다.
22. 검색창에 `week3-2-security-group`을 입력하여 필터링합니다.
23. 다음 5개의 보안 그룹이 표시되는지 확인합니다:
...
24. ALB-SG, Web-SG, App-SG, DB-SG를 각각 선택하고 하단의 **Inbound rules** 탭을 확인합니다.
...
25. AWS Management Console에 로그인한 후 상단 검색창에 `VPC`을 입력하고 선택합니다.
26. 왼쪽 메뉴에서 **Security groups**를 선택합니다.
27. `week3-2-security-group-ALB-SG` 보안 그룹을 선택합니다.
28. 하단의 **Inbound rules** 탭을 선택합니다.
29. [[Edit inbound rules]] 버튼을 클릭합니다.
30. [[Add rule]] 버튼을 클릭합니다.
31. 첫 번째 규칙을 설정합니다:
	- **Type**에서 `HTTP`를 선택합니다.
	- **Source**에서 `0.0.0.0/0`을 입력합니다.
	- **Description**에 `Allow HTTP from internet`를 입력합니다.
32. [[Add rule]] 버튼을 다시 클릭합니다.
33. 두 번째 규칙을 설정합니다:
	- **Type**에서 `HTTPS`를 선택합니다.
	- **Source**에서 `0.0.0.0/0`을 입력합니다.
	- **Description**에 `Allow HTTPS from internet`를 입력합니다.
34. [[Save rules]] 버튼을 클릭합니다.
...
35. `week3-2-security-group-Web-SG` 보안 그룹을 선택합니다.
36. 하단의 **Inbound rules** 탭을 선택합니다.
37. [[Edit inbound rules]] 버튼을 클릭합니다.
...
45. `week3-2-security-group-App-SG` 보안 그룹을 선택합니다.
46. 하단의 **Inbound rules** 탭을 선택합니다.
47. [[Edit inbound rules]] 버튼을 클릭합니다.
48. [[Add rule]] 버튼을 클릭합니다.
49. 첫 번째 규칙을 설정합니다:
...
50. [[Add rule]] 버튼을 다시 클릭합니다.
51. 두 번째 규칙을 설정합니다:
	- **Type**에서 `SSH`를 선택합니다.
	- **Source**에서 `Custom`을 선택한 후, 검색 필드에 `Web-SG`를 입력하여 `week3-2-security-group-Web-SG`를 찾아 선택합니다.
	- **Description**에 `Allow SSH from Web tier`를 입력합니다.
52. [[Save rules]] 버튼을 클릭합니다.
...
53. `week3-2-security-group-DB-SG` 보안 그룹을 선택합니다.
54. 하단의 **Inbound rules** 탭을 선택합니다.
55. [[Edit inbound rules]] 버튼을 클릭합니다.
56. [[Add rule]] 버튼을 클릭합니다.
57. 첫 번째 규칙을 설정합니다:
...
59. AWS Management Console에서 상단 검색창에 `EC2`를 입력하고 선택합니다.
60. 왼쪽 메뉴에서 **Instances**를 선택합니다.
61. 검색창에 `week3-2-security-group-Web-Instance`를 입력합니다.
62. `week3-2-security-group-Web-Instance` 인스턴스를 선택합니다.
63. [[Connect]] 버튼을 클릭합니다.
64. **Session Manager** 탭을 선택합니다.
65. [[Connect]] 버튼을 클릭합니다.

> [!NOTE]
> 새 브라우저 탭에서 터미널 세션이 열립니다. 인스턴스가 시작된 지 2-3분 후에 Session Manager를 사용할 수 있습니다.

66. Session Manager 터미널에서 다음 명령어를 실행하여 App 인스턴스의 Private IP를 확인합니다:
...
67. App 서버(포트 8080)로 연결을 테스트합니다:
...
69. Web 서버에서 DB 서버(포트 3306)로 직접 연결을 시도합니다:
...
70. EC2 콘솔로 이동합니다.
71. `week3-2-security-group-App-Instance` 인스턴스를 선택합니다.
72. [[Connect]] > **Session Manager** > [[Connect]]를 클릭합니다.
73. App 서버에서 DB 서버로 연결을 테스트합니다:
...
75. 왼쪽 메뉴에서 **Network ACLs**를 선택합니다.
76. [[Create network ACL]] 버튼을 클릭합니다.
77. **Name**에 `Public-NACL`을 입력합니다.
78. **Amazon VPC**에서 AWS CloudFormation이 생성한 Amazon VPC를 선택합니다.

> [!TIP]
> Amazon VPC ID는 메모장에 저장한 VpcId와 일치해야 합니다. Amazon VPC 이름에 "week3-2-security-group"이 포함되어 있는지 확인합니다.

79. **Tags** 섹션에서 [[Add tag]] 버튼을 클릭한 후 다음 태그를 추가합니다:
...
80. [[Create network ACL]] 버튼을 클릭합니다.

### 인바운드 규칙 설정

81. 생성된 `Public-NACL`을 선택합니다.
82. 하단의 **Inbound rules** 탭을 선택합니다.
83. [[Edit inbound rules]] 버튼을 클릭합니다.
84. 다음 4개의 인바운드 규칙을 [[Add new rule]] 버튼을 클릭하여 순서대로 추가합니다:
...
86. 하단의 **Outbound rules** 탭을 선택합니다.
87. [[Edit outbound rules]] 버튼을 클릭합니다.
88. 다음 3개의 아웃바운드 규칙을 [[Add new rule]] 버튼을 클릭하여 순서대로 추가합니다:
...
90. [[Create network ACL]] 버튼을 다시 클릭합니다.
91. **Name**에 `Private-NACL`을 입력합니다.
92. **Amazon VPC**에서 AWS CloudFormation이 생성한 Amazon VPC를 선택합니다.

> [!TIP]
> Amazon VPC ID는 메모장에 저장한 VpcId와 일치해야 합니다.

93. **Tags** 섹션에서 [[Add tag]] 버튼을 클릭한 후 다음 태그를 추가합니다:
...
94. [[Create network ACL]] 버튼을 클릭합니다.
95. 생성된 `Private-NACL`을 선택합니다.
96. 하단의 **Inbound rules** 탭을 선택합니다.
97. [[Edit inbound rules]] 버튼을 클릭합니다.
98. [[Add new rule]] 버튼을 클릭합니다.
99. 첫 번째 규칙을 설정합니다 (Amazon VPC 내부 트래픽):
	- **Rule number**에 `100`을 입력합니다.
	- **Type**에서 `All traffic`을 선택합니다.
	- **Source**에 `10.0.0.0/16`을 입력합니다.
	- **Allow/Deny**에서 `Allow`를 선택합니다.
100. [[Add new rule]] 버튼을 다시 클릭합니다.
101. 두 번째 규칙을 설정합니다 (임시 포트):
...
103. 하단의 **Outbound rules** 탭을 선택합니다.
104. [[Edit outbound rules]] 버튼을 클릭합니다.
105. [[Add new rule]] 버튼을 클릭합니다.
106. 첫 번째 규칙을 설정합니다:
    - **Rule number**에 `100`을 입력합니다.
    - **Type**에서 `All traffic`을 선택합니다.
...
108. `Public-NACL`을 선택합니다.
109. 하단의 **Subnet associations** 탭을 선택합니다.
110. [[Edit subnet associations]] 버튼을 클릭합니다.
111. `week3-2-security-group-Public-Subnet-A`와 `week3-2-security-group-Public-Subnet-C`를 체크합니다.
112. [[Save changes]] 버튼을 클릭합니다.
113. `Private-NACL`을 선택합니다.
114. 하단의 **Subnet associations** 탭을 선택합니다.
115. [[Edit subnet associations]] 버튼을 클릭합니다.
116. 프라이빗 서브넷들을 체크합니다.
...
118. Web 인스턴스에 Session Manager로 접속합니다 (태스크 5 참고).
119. App 서버로 연결을 테스트합니다:
...
120. VPC 콘솔로 이동합니다.
121. 왼쪽 메뉴에서 **Network ACLs**를 선택합니다.
122. `Private-NACL`을 선택합니다.
123. 하단의 **Inbound rules** 탭을 선택합니다.
124. [[Edit inbound rules]] 버튼을 클릭합니다.
125. [[Add new rule]] 버튼을 클릭합니다.
126. 새 규칙을 설정합니다 (포트 8080 차단):
...
128. Web 인스턴스 Session Manager로 이동합니다.
129. App 서버로 다시 연결을 시도합니다:
...
130. VPC 콘솔에서 `Private-NACL`의 **Inbound rules**를 다시 편집합니다.
131. 규칙 50을 삭제합니다:
	- 규칙 50 옆의 [[Remove]] 버튼을 클릭합니다.
132. [[Save changes]] 버튼을 클릭합니다.
133. Web 인스턴스 Session Manager로 이동합니다.
134. App 서버로 다시 연결을 시도합니다:
...
> [!NOTE]
> 스택 삭제가 완료될 때까지 기다립니다. 스택이 완전히 삭제되면 스택 목록에서 사라집니다. 만약 "DELETE_FAILED"가 표시되면 **Events** 탭에서 오류 원인을 확인하고, 보안 그룹 간 참조 관계를 수동으로 제거한 후 스택 삭제를 다시 시도합니다.

35. 스택 목록 페이지로 돌아가서 `week3-2-security-group-stack` 스택이 목록에서 사라졌는지 확인합니다.
...
36. AWS Management Console에서 `Resource Groups & Tag Editor`로 이동합니다.
37. 왼쪽 메뉴에서 **Tag Editor**를 선택합니다.
38. **Regions**에서 `ap-northeast-2`를 선택합니다.
39. **Resource types**에서 `All supported resource types`를 선택합니다.
40. **Tags** 섹션에서 다음을 입력합니다:
	- **Tag key**: `Name`
	- **Optional tag value**에 `week3-2`를 입력합니다.
