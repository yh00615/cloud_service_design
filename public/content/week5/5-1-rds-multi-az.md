...
22. [[Create database]] 버튼을 클릭합니다.
23. **Engine type**에서 `MySQL`을 선택합니다.
24. **Engine version**에서 콘솔에서 기본 선택되는 MySQL 버전을 사용합니다.
...
26. **Availability and durability** 섹션에서 **Deployment options**를 확인합니다.
27. `Multi-AZ DB instance deployment (2 instances)`를 선택합니다.
...
29. **Credentials Settings** 섹션에서 **Master username**에 `admin`을 입력합니다.
...
34. **Storage type**에서 `General Purpose SSD (gp2)`를 선택합니다.
35. **Allocated storage**에 기본값 `200`이 표시됩니다. 이를 `20` GiB로 수정합니다.
...
71. **Events** 섹션에서 다음 페일오버 관련 이벤트들을 확인합니다:
...
80. **Monitoring** 섹션에서 `Database Insights - Standard`를 선택하고 **Enhanced Monitoring**을 체크 해제합니다.
81. [[Create read replica]] 버튼을 클릭합니다.
...
82. `mysql-lab-instance`를 선택합니다.
83. **Actions** > `Take snapshot`을 선택합니다.
84. **Snapshot name**에 `mysql-lab-snapshot-manual`을 입력합니다.
85. [[Take snapshot]] 버튼을 클릭합니다.
86. 왼쪽 메뉴에서 **Snapshots**를 선택하여 스냅샷 생성 진행 상황을 확인합니다.
...
88. **Maintenance & backups** 탭을 선택합니다.
89. **Automated backups** 섹션에서 다음을 확인합니다:
...
24. `mysql-lab-snapshot-manual`을 선택합니다.
...
30. `week5-1-rds-stack` 스택을 선택합니다.
31. [[Delete stack]] 버튼을 클릭합니다.
32. 확인 창에서 스택 이름 `week5-1-rds-stack`을 입력합니다.
33. [[Delete stack]] 버튼을 클릭합니다.
34. 스택 상태가 "DELETE_IN_PROGRESS"로 변경됩니다.
...
37. **Regions**에서 `ap-northeast-2`를 선택합니다.
38. **Resource types**에서 `All supported resource types`를 선택합니다.
39. **Tags** 섹션에서 다음을 입력합니다:
    - **Tag key**: `Week`
    - **Tag value**: `5-1`
40. [[Search resources]] 버튼을 클릭합니다.
41. 검색 결과에 리소스가 표시되지 않는지 확인합니다.
