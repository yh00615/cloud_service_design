...
3. 템플릿 구조를 확인합니다:
   - **Parameters**: EnvironmentName, VpcCIDR, PublicSubnetCIDR, InstanceType, LatestAmiId, ProjectTag, WeekTag, CreatedByTag
   - **Resources**: VPC, InternetGateway, AttachGateway, PublicSubnet, PublicRouteTable, DefaultPublicRoute, SubnetRouteTableAssociation, WebServerSecurityGroup, WebServerRole, WebServerInstanceProfile, WebServer
...
> - **EnableDnsHostnames**: `true`로 설정하면 퍼블릭 IP가 있는 인스턴스에 DNS 호스트 이름 자동 할당
> - **EnableDnsSupport**: `true`로 설정하면 VPC 내에서 Amazon 제공 DNS 서버를 사용한 DNS 확인 가능 (EnableDnsHostnames 사용 시 반드시 `true` 필요)
...
34. **VPC** 리소스의 Physical ID (vpc-xxxxx)를 클릭합니다.
35. VPC 콘솔로 이동하여 VPC 상세 정보를 확인합니다.
36. 브라우저 뒤로가기 버튼을 클릭하여 AWS CloudFormation 콘솔로 이동합니다.
37. **WebServer** 리소스의 Physical ID (i-xxxxx)를 클릭합니다.
38. EC2 콘솔로 이동하여 인스턴스 상세 정보를 확인합니다.
...
> - VPCId는 Export되어 있어 다른 스택에서 이 VPC를 참조 가능
...
**!Select**: 리스트에서 특정 인덱스의 값을 선택

```yaml
AvailabilityZone: !Select [0, !GetAZs '']
# 첫 번째(인덱스 0) 가용 영역을 선택
```

**!GetAZs**: 현재 리전의 모든 가용 영역 목록을 반환

```yaml
!GetAZs ''
# 빈 문자열('')은 현재 리전을 의미
# ap-northeast-2의 경우 [ap-northeast-2a, ap-northeast-2b, ...] 반환
```
...
**CidrBlock**: VPC의 IP 주소 범위를 CIDR 표기법으로 지정

```yaml
CidrBlock: !Ref VpcCIDR # 예: 10.0.0.0/16
```

**EnableDnsHostnames**: VPC 내 인스턴스에 퍼블릭 DNS 호스트 이름을 자동 할당

- `true`로 설정하면 퍼블릭 IP가 있는 인스턴스에 DNS 이름이 부여됨 (예: ec2-x-x-x-x.ap-northeast-2.compute.amazonaws.com)

**EnableDnsSupport**: VPC 내에서 DNS 확인 기능을 활성화

- `true`로 설정하면 Amazon 제공 DNS 서버(169.254.169.253)를 사용하여 DNS 쿼리 가능
- EnableDnsHostnames를 사용하려면 반드시 `true`여야 함
...
**DestinationCidrBlock**: 라우팅 대상 IP 범위

- `0.0.0.0/0`: 모든 외부 트래픽을 지정된 게이트웨이로 라우팅 (기본 라우트)

### 보안 그룹

**GroupName**: 보안 그룹의 이름

**GroupDescription**: 보안 그룹에 대한 설명 (필수 속성)
...
**SecurityGroupIds**: VPC 내 인스턴스에 보안 그룹을 연결할 때 사용 (보안 그룹 ID 목록)
...
**IamInstanceProfile**: 인스턴스에 AWS IAM 역할을 연결하여 AWS 서비스에 접근할 수 있도록 설정

### AWS IAM 역할

**AssumeRolePolicyDocument**: 이 역할을 맡을 수 있는 서비스를 정의하는 신뢰 정책
...
AssumeRolePolicyDocument:
  Statement:
    - Effect: Allow
      Principal:
        Service: ec2.amazonaws.com # EC2 서비스가 이 역할을 맡을 수 있음
      Action: sts:AssumeRole
```

**ManagedPolicyArns**: 역할에 연결할 AWS 관리형 정책 목록

**InstanceProfile**: Amazon EC2 인스턴스에 AWS IAM 역할을 연결하기 위한 컨테이너 리소스. EC2에 직접 역할을 연결할 수 없으므로 InstanceProfile을 통해 연결함

### Outputs 및 Export

**Export**: 출력값을 다른 스택에서 참조할 수 있도록 내보내기
...
**!ImportValue**: 다른 스택에서 Export된 값을 가져올 때 사용
...
이 템플릿에서는 Parameters로 태그 값을 받아 모든 리소스에 일관되게 적용하는 패턴을 사용합니다.

```yaml
# Parameters에서 태그 값 정의
ProjectTag:
  Type: String
  Default: 'AWS-Lab'
...
- 태그 값을 한 곳(Parameters)에서 관리하므로 변경이 용이함
- 모든 리소스에 동일한 태그를 적용하여 비용 추적 및 리소스 관리에 활용
