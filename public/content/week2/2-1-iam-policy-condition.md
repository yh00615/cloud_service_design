...
7. AWS IAM 콘솔로 이동합니다.
8. 왼쪽 메뉴에서 **Users**를 선택합니다.
9. 사용자 목록에서 `condition-test-user`를 검색합니다.
10. `condition-test-user`를 클릭합니다.
11. **Security credentials** 탭을 선택합니다.
...
17. 확인 창에서 **Access key ID**를 입력 필드에 입력합니다.
18. [[Delete]] 버튼을 클릭합니다.
...
19. AWS CloudShell로 이동합니다.
20. 다음 명령어를 실행하여 condition-test 프로파일이 존재하는지 확인합니다:
...
21. 다음 명령어를 실행하여 credentials 파일에서 프로파일을 삭제합니다:

```bash
sed -i.bak '/^\[condition-test\]/,/^\[/{/^\[condition-test\]/d;/^\[/!d;}' ~/.aws/credentials
```

22. 다음 명령어를 실행하여 config 파일에서 프로파일을 삭제합니다:
...
25. AWS IAM 콘솔로 이동합니다.
26. 왼쪽 메뉴에서 **Users**를 선택합니다.
...
32. 왼쪽 메뉴에서 **Policies**를 선택합니다.
33. 정책 목록에서 `S3MFARequiredPolicy`를 검색합니다.
34. `S3MFARequiredPolicy` 정책 왼쪽의 라디오 버튼을 선택합니다.
...
45. 버킷 목록에서 해당 버킷이 더 이상 표시되지 않는지 확인합니다.
...
46. AWS Management Console에 로그인한 후 상단 검색창에 `Resource Groups & Tag Editor`을 입력하고 선택합니다.
47. 왼쪽 메뉴에서 **Tag Editor**를 선택합니다.
